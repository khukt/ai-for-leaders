document.addEventListener('DOMContentLoaded', function () {
  ...
  var aiActTimelineConfigs = {
    entry: {
      eyebrow: 'Entry into force',
      title: 'The EU AI Act became law at regional scale',
      summary: 'This date starts the phased compliance clock. It does not mean every duty applies at once, but it does mean leaders should already stop treating the Act as a distant issue.',
      changed: 'The Act entered into force and the implementation timetable formally began.',
      care: 'Management should begin inventory, scoping, ownership, and policy preparation before later obligations arrive.',
      action: 'Build a view of which systems may fall into scope and who is accountable for each one.',
      question: 'Do we know which uses are likely to face stronger obligations before the final implementation date arrives?',
      rule: 'Key idea: entry into force is the start of management preparation, not the point to wait passively for later dates.'
    },
    prohibited: {
      eyebrow: 'First obligations apply',
      title: 'Prohibited practices and AI literacy duties moved from theory into force',
      summary: 'By February 2, 2025, some uses were already forbidden and organisations needed to take AI literacy seriously rather than leave it implicit.',
      changed: 'Prohibited practices began to apply, and AI literacy obligations started to matter.',
      care: 'This is where a weak use-case screen or poor workforce guidance can turn into immediate exposure.',
      action: 'Screen for prohibited uses and make sure staff and managers understand the rules around approved use.',
      question: 'Could any current or planned use be in a banned category, and are staff equipped to recognize that early?',
      rule: 'Key idea: the first date that matters is not the final compliance date. Some obligations already arrived much earlier.'
    },
    gpai: {
      eyebrow: 'GPAI obligations begin',
      title: 'General-purpose AI governance started to affect model providers and dependent organisations',
      summary: 'By August 2, 2025, governance rules and obligations for general-purpose AI models were already becoming operationally relevant.',
      changed: 'Governance and GPAI model obligations became applicable.',
      care: 'Leaders relying on foundation-model vendors now need clearer visibility into provider choice, documentation, and dependency.',
      action: 'Review which tools rely on general-purpose models and what evidence, terms, and governance you require from suppliers.',
      question: 'Do we know where our external AI tools depend on model providers whose obligations are changing?',
      rule: 'Key idea: model-provider governance affects deployers indirectly through procurement, documentation, and dependency.'
    },
    full: {
      eyebrow: 'Main operational deadline',
      title: 'Most of the AI Act becomes fully applicable on August 2, 2026',
      summary: 'For many organisations, this is the major operational date. By then, governance should be visible, not still in design.',
      changed: 'Most of the Act became fully applicable.',
      care: 'This is the point where weak inventory, unclear ownership, or incomplete documentation become much harder to defend.',
      action: 'Make sure material systems have classification, ownership, review logic, documentation, and escalation before this date.',
      question: 'If regulators or auditors asked today, could we show how our material AI uses are governed?',
      rule: 'Key idea: full applicability is the point by which governance should already be functioning, not the point to start planning.'
    },
    extended: {
      eyebrow: 'Extended product timeline',
      title: 'Some regulated high-risk products received a longer implementation runway',
      summary: 'By August 2, 2027, certain high-risk systems embedded in regulated products reach their longer-tail deadline.',
      changed: 'Some high-risk systems embedded in regulated products follow a longer timetable.',
      care: 'The extension should not be read as relief for everyone. It applies to narrower product contexts, not to general management delay.',
      action: 'Confirm whether any of your systems sit in this narrower category before assuming you have extra time.',
      question: 'Are we treating a specific extension as if it were a general reason to postpone broader compliance work?',
      rule: 'Key idea: an extended deadline for some regulated products should not become an excuse for overall delay.'
    }
  };
  ...
  document.querySelectorAll('[data-interactive-diagram="ai-act-timeline"]').forEach(function (diagram) {
    var tabs = diagram.querySelectorAll('[data-ai-act-timeline-tab]');
    var eyebrowTarget = diagram.querySelector('[data-ai-act-timeline-eyebrow]');
    var titleTarget = diagram.querySelector('[data-ai-act-timeline-title]');
    var summaryTarget = diagram.querySelector('[data-ai-act-timeline-summary]');
    var changedTarget = diagram.querySelector('[data-ai-act-timeline-changed]');
    var careTarget = diagram.querySelector('[data-ai-act-timeline-care]');
    var doTarget = diagram.querySelector('[data-ai-act-timeline-do]');
    var questionTarget = diagram.querySelector('[data-ai-act-timeline-question]');
    var ruleTarget = diagram.querySelector('[data-ai-act-timeline-rule]');

    function activate(key) {
      var config = aiActTimelineConfigs[key];
      if (!config) {
        return;
      }

      tabs.forEach(function (tab) {
        var isActive = tab.getAttribute('data-ai-act-timeline-tab') === key;
        tab.classList.toggle('is-active', isActive);
        tab.setAttribute('aria-pressed', isActive ? 'true' : 'false');
        tab.tabIndex = isActive ? 0 : -1;
      });
      ...
      titleTarget.textContent = config.title;
      summaryTarget.textContent = config.summary;
      changedTarget.textContent = config.changed;
      careTarget.textContent = config.care;
      doTarget.textContent = config.action;
      questionTarget.textContent = config.question;
      ruleTarget.textContent = config.rule;
    }
    ...
    tabs.forEach(function (tab) {
      tab.addEventListener('click', function () {
        activate(tab.getAttribute('data-ai-act-timeline-tab'));
      });
      ...
      tab.addEventListener('keydown', function (event) {
        ...
        tabs[nextIndex].focus();
        activate(tabs[nextIndex].getAttribute('data-ai-act-timeline-tab'));
      });
    });
